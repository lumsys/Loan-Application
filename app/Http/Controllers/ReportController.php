        ...
        if (!empty($start_date)) {
            ...
            if ($request->loan_product_id == "-1") {
                ...
                $data = Loan::where('loan_product_id', $request->loan_product_id)->where('branch_id',
                    session('branch_id'))->where('status', 'pending')->whereBetween('release_date',
                    [$start_date, $end_date])->get();
...
/*
        return view('loan_report.disbursed_loans',
            ...
            compact('start_date',
                'end_date', 'data', 'user_id', 'loan_product_id', 'users', 'loan_products', 'banks'));
*/
                return view('loan_report.disbursed_loans',
            compact('start_date',
                'end_date', 'user_id', 'loan_product_id', 'users', 'loan_products'));
...
*/
    ...
    public function provisioning(Request $request)
    {
        ...
        $end_date = $request->end_date;
     /*   
        $apps =Loan::where('release_date','<=',$end_date)->where('branch_id',
        session('branch_id'))->where('status','disbursed')->orderBy('release_date','asc')->get();
        ...
        if (!empty($end_date)) {
            ...
            foreach (Loan::where('release_date', '<=', $end_date)->where('branch_id',
                ...
                if (!empty($key->bank_account)) {
                    $bank = $key->bank_account->name;
                } else {
                    $bank = "";
                }

                if (!empty($key->account_number)) {
