...
//         $loans = loan::where('borrower_id', $request->session()->get('uid'))->get();
//         // dd($loans);
//        foreach($loans as $loan)
// {

//         $cuxers = $loan->loan_product_id;
//         // dd(date("Y-m-d",strtotime(Carbon::now()->subDays(30))));
        
//     if ($cuxers== 1) {
//         if($loan->release_date >=  date("Y-m-d",strtotime(Carbon::now()->subDays(30)))){
//             Flash::warning("Permission Denied");
//                  return redirect()->back();
//                     }         
//     }else
//     if (($cuxers== 2) && ($loan->release_date >=  date("Y-m-d",strtotime(Carbon::now()->subDays(60))))){
//             Flash::warning("Permission Denied");
//             return redirect()->back();            
//         }else
//     if (($cuxers== 3) && ($loan->release_date >=  date("Y-m-d",strtotime(Carbon::now()->subDays(80))))){
//             Flash::warning("Permission Denied");
//             return redirect()->back();            
//         }else
//     if (($cuxers== 4) && ($loan->release_date >=  date("Y-m-d",strtotime(Carbon::now()->subDays(120))))){
//             Flash::warning("Permission Denied");
//             return redirect()->back();            
//         }else
//     if (($cuxers== 5) && ($loan->release_date >=  date("Y-m-d",strtotime(Carbon::now()->subDays(120))))){
//             Flash::warning("Permission Denied");
//             return redirect()->back(); 
...
//}        
        ...
        foreach (LoanProduct::all() as $key) {
            $products[$key->id] = $key->name . '(' . round($key->default_loan_duration) . '-' . ($key->default_loan_duration_type) . ')';
        }
/*
        $durations = array();
        foreach (LoanProduct::all() as $key) {
            $durations[$key->id] = $key->name . '(' . round($key->default_loan_duration) . '-' . round($key->default_loan_duration_type) . ')';
        }
*/
        $branches = array();
    ...
    public function storeApplication(Request $request)
    {
        ...
        $application->bank_account_id = $request->bank_account_id;
        //$application->interest = $request->interest;
        $application->account_number = $request->input('account_number');
        $application->bank = $request->input('bank');
        $application->amount = $request->amount;
        $application->charges = $request->amount*0.01;
        $uxer = Borrower::find($request->session()->get('uid'));
        $check = loanProduct::where('id', $request->loan_product_id)->first();
        $checkRe = $check->default_loan_duration;
        //dd($checkRe);
        $max_borrow = $uxer->total_salary*0.33*$checkRe;
        //dd($max_borrow);
            if($application->amount > $max_borrow){
                return back()->with('amount_error','Invalid amount entered')->withInput();
}
//continue and save
        $application->notes = $request->notes;
        $application->save();
   /*     
        $user = Borrower::find($request->session()->get('uid'));
        $email = $user['email'];
        $messageData = ['email'=>$email = $user['email'],
         'name'=>$user['name']];
        Mail::send('email.loanApply', $messageData,function($message) use ($email){
        $message->to($email)->subject('You applied for loan');
        });
*/
